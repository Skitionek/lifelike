...
### Changed
- **CI linting**: all linters in `fast-lint` and MegaLinter now run with auto-fixes enabled; MegaLinter fixes are opened as a separate PR using `peter-evans/create-pull-request` (`APPLY_FIXES_MODE: pull_request`).
