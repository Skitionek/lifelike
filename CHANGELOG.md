...
- **User file content** reads and writes now go through the `FileStorageService` libcloud abstraction. The default `PostgreSQLStorageDriver` stores bytes in `files_content.raw_file` (no schema change, no external service required). Switching to Azure Blobs, S3, or GCS only requires setting `FILE_STORAGE_PROVIDER` / `FILE_STORAGE_KEY` / `FILE_STORAGE_SECRET` env vars — no code changes. The hex-encoded SHA-256 checksum of each file is used as the libcloud `object_name` (exposed as `FileContent.revision`), mapping the content-addressed storage key directly to the file revision concept.
- `FileContent.get_bytes()` always routes through `FileStorageService.retrieve()` inside a Flask app context; `PostgreSQLStorageDriver` is the only component that accesses `files_content.raw_file` directly. Removed the silent broad-exception catch and the POSTGRESQL short-circuit that bypassed the storage abstraction.
- `folder_annotation_service._lookup_annotations_config()` now retrieves file bytes via `FileContent.get_bytes()` instead of issuing a raw `db.session.query(FileContent.raw_file)` query.
- `files_queries.get_all_files_and_content_by_id()` no longer selects `FileContent.raw_file` directly (callers should use `FileContent.get_bytes()`).
- `FileVersionSchema` now includes a `revision` field — the hex-encoded SHA-256 of that version's content — so clients can resolve any historical file version directly via the storage service.
