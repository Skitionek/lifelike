...
- **`AnnotationsFileTypeProvider`**: registered file-type provider for `.annotations` MIME type. Validates uploaded JSON against the schema; triggers a concurrent refresh of the `file_effective_annotation_config` materialized view after each commit.
- **`FolderAnnotationService`** (`neo4japp/services/annotations/folder_annotation_service.py`): queries the `file_effective_annotation_config` materialized view (O(1) lookup); falls back to walking the ancestor chain and reading `files_content.raw_file` when the view is unavailable.
